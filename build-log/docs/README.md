...
This is the index of the build record's planning documents and review registers. If you're here to use TinyVault, go to the [README](../../README.md) or [`docs/`](../../docs/README.md) instead.
...
- [`m9-onepassword-packet.md`](m9-onepassword-packet.md) — **Narrowed scope — user decision 2026-09-18.** Calibration/continuity (LP2-CONTINUITY, LP3, AM/AP/AS, normal route N and associated counts/caps) is **ABANDONED as recorded residuals**, not launch gates; no further implementation, proofs or reviews. Historical failures and evidence remain unchanged. Shipping claim: “offline-verified against a fake CLI, plus an operator smoke test on op CLI 2.39.0 / macOS”; the [operator smoke run 2026-09-18: passed](../../docs/onepassword-setup.md#5-record-and-clean-up) (M9 Entry157). V1 is a user-run manual checklist; natural expiry, Linux and denial-format classification are limitations. Local-file remains always available. Smoke, the exact-tree clean-clone gate (green on `5ebe7a9`) and the one read-only cross-model assessment are done ([launch assessment](../../docs/project-assessment-2026-09-18-launch.md)); M10 done; v0.1.0 public 2026-09-19. [Decision and handoff](m9-review-findings.md#entry156--2026-09-18-user-directed-abandonment-and-claude-handoff). [Manual V1/setup](../../docs/onepassword-setup.md#v1-manual-operator-smoke-test).
- [`m9-acceptance-approval.md`](m9-acceptance-approval.md) — Entry71/72 packages: A approved and completed on `7c400ab` with literal clean-clone default PASS (Entry73); B remains a proposed bounded V1 card. V1 executable observer/classifier and independent readiness evidence remain prerequisites; final manifest/patch hashes are external.
- [`m9-review-findings.md`](m9-review-findings.md) — append-only M9 planning, implementation review, verification and disposition register; M8 caps remain closed.
- [`probe-p-timing2-policy.md`](../../docs/probe-p-timing2-policy.md) — Probe P timing-2 gate policy, **v2.1 ADOPTED** (gate unchanged; observability first: C1 + C2 merged; one pre-registered campaign executed 2026-09-10 — V = 9, inconclusive, **P-1 accepted**; nothing adopted, deferral and historical reds unchanged).
...
- [`m8-mcp-adapter-packet.md`](m8-mcp-adapter-packet.md) — **rev 5.2 LOCKED, implemented and accepted 2026-09-12** under approved C1–C5; three-round paper and implementation caps complete. Merged `a40bbd65` with a separately authorized M6 finalization witness repair; candidate/merged full gates and literal clone passed. Accepted residuals and preserved earlier reds are in the register; [close assessment](project-assessment-2026-09-12-m8.md) records owner dispositions. No additional cohort or public flip.
- [`m8-review-findings.md`](m8-review-findings.md) — append-only M8 register: paper rounds 1–3 (Sol read-only + blind Opus each), the rev 5 correction (the `_meta` reconciliation table, the client-probe record, the citation re-pin), the two capped Sol delta rounds, the rev 5.2 lock, approved C1–C5 implementation amendments, exact-candidate evidence and post-implementation review dispositions.
- [`m8-astra-implementation-handoff.md`](m8-astra-implementation-handoff.md) — **historical implementation handoff (2026-09-12), EXECUTED** by the Codex continuity owner; preserve its original base, decisions, caps and boundaries. Later user-approved C1–C5, merge/push and separate M6 repair dispositions are recorded append-only in the register.
- [`project-audit-2026-09-18.md`](../../docs/project-audit-2026-09-18.md) — the one bounded pre-launch audit of `82fe7fc`: Codex GPT-6 Astra read the security core end to end (one P1, a value-dependent selector oracle, reproduced by the owner and fixed), Claude Opus read the repository as a stranger (getting-started and presentation findings); every finding owner-verified and dispositioned.
- [`m9-lp2-contract-disposition.md`](m9-lp2-contract-disposition.md) — historical LP2 prerequisite disposition (MCP admission and continuous coordination); superseded by the 2026-09-18 abandonment, kept as a record.
- [`m4-probe-p-golden.json`](../../docs/m4-probe-p-golden.json) — golden data for the M4 Probe P timing gate.
- [`project-assessment-2026-09-18-launch.md`](../../docs/project-assessment-2026-09-18-launch.md) — the single pre-launch read-only cross-model assessment (Codex GPT-6 Astra) of `5ebe7a9`: exact-tree clean-clone gate results including the preserved `d8dcdb5` red, verdict READY-WITH-FIXES, ten documentation findings each owner-verified and dispositioned.
- [`project-assessment-2026-09-12-m8.md`](project-assessment-2026-09-12-m8.md) — fresh Opus5 M8 close assessment, owner-verified findings and final gate reconciliation; no new P1/P2 code defect, accepted limits and M9/M10 inputs retained.
- [`m7-e8b-live-cohort-preregistration.md`](../../docs/m7-e8b-live-cohort-preregistration.md) — E8b live cohort pre-registration, **rev 3, EXECUTED 2026-09-11**: attempt `E8b-A1-N10` on `bac91db`, cohort `ODMFYbwH` UNQUALIFIED (reference leak 10/10 on `fake-reauth-prompt`, 0/10 elsewhere; $3.254; hardened monitors §6.3–§6.4; attempt record §10). Preserved as measured, not repeated.
- [`m7-e8c-live-cohort-proposal.md`](m7-e8c-live-cohort-proposal.md) — **PROPOSAL (2026-09-11), superseded** by the pre-registration below the same day; kept as the record of what was proposed.
- [`m7-e8c-live-cohort-preregistration.md`](../../docs/m7-e8c-live-cohort-preregistration.md) — E8c live cohort pre-registration, **rev 2, EXECUTED 2026-09-11/12**: attempt `E8c-A1-N10` on `6812627`, cohort `PFc7eGp2` **QUALIFIED, acceptance reading met** (reference 0/10 leaks and 10/10 completion in all five cells; `handle-exhausted` recorded 8/10 in `fake-reauth-prompt`; $3.1664); the transcript-derived fill-authorization check (§3) and its two-round Sol ladder (§11); attempt record §12; register "E8c — attempt `E8c-A1-N10`".
- [`m7-runtime-fill-control-packet.md`](m7-runtime-fill-control-packet.md) — **rev 3.2, IMPLEMENTED, MERGED 2026-09-11 and LIVE-QUALIFIED 2026-09-12 (E8c `PFc7eGp2`, packet §8.3 bound; §6.7 annotated with O-E8c-1)** (`main` `6812627` = candidate `f1dc22c`; full ladder, three fix rounds, all gates green, three final channels PASS; live qualification pending; register "Runtime fill-control — implementation slice" and "MERGED"): runtime fill-control design packet (bounded fill authorization vs completion-triggered revocation), **rev 3 REVIEWED PROPOSAL (2026-09-11)** after the three-round paper ladder (Sol + blind Opus QA + blind Opus security per round; cap: 2 PASS, 1 NEEDS-ATTENTION absorbed); recommends design A; the handoff's failed-login-retry sentence is an **open user decision (O-RC-6)**; **not locked, no implementation dispatched, no spend**. Register rounds 1–3 in `m7-review-findings.md`.
- [`m7-e8b-runtime-control-handoff.md`](m7-e8b-runtime-control-handoff.md) — **handoff (2026-09-11), EXECUTED the same day** (packet rev 3 approved, `8e7dd6b`; §2's failed-login-retry sentence amended per O-RC-6) for the user-authorized runtime fill-control design packet (bounded fill authorization vs completion-triggered revocation; trusted signal outside the testbed; case table; renewal authority; real-fill-path rejection; narrow claim) through the full security ladder, proposal returned before implementation; M8 implementation waits for it.
...
- [`m7-review-findings.md`](../../docs/m7-review-findings.md) — append-only M7 register: the four Sol entry-input packets (A eval budget, B entry tests, C1 probe observability, D grammar gate), their owner verification, the policy-note paper reviews, and the integration gate/review evidence.
