...
## Entry157 — 2026-09-18: V1 manual operator smoke, run by the user — PASSED

Owner: Claude (session `2026-09-18-ship-path-claude`). The user ran the [manual checklist](../../docs/onepassword-setup.md#v1-manual-operator-smoke-test) in their own terminal against the real 1Password service with a throwaway vault, one synthetic Login and a read-only service account. Claude never held the token, the password, the config or the IDs; it saw only the tool replies the user pasted, file metadata, and the saved transcript after the secret grep was clean. No model or agent was in the loop: requests were pasted by hand into `dist/tinyvault-mcp.mjs`, so saved stdout is the model-visible surface.

**Environment:** macOS 15.6.1 (24G90) arm64; op CLI 2.39.0 (Homebrew cask), 41,016,304 bytes, SHA-256 equal to the setup pin; tree `f60bb23` plus the uncommitted checklist rewrite (docs only).

**Results:** list returned exactly one item with the configured label and `available:true`, the configured non-existent item ID was omitted, and no provider title/username/URL/ID appeared. Fill returned `{"ok":true,"filled":["password"]}`; the throwaway login page received one POST of the expected length; the snapshot showed `Signed in.`. An unknown handle was refused `handle-unavailable` with no POST. A bad token and, in a fresh process, a revoked token each returned the fixed `Vault operation failed` / `isError:true` with no provider text. The SHA-256 prefix of the operator's pasted password equalled the login page's (positive control), then `grep -rlF` for the password and for the token over the transcript and logs each exited 1. The same grep found a known-present string in the 19-reply transcript.

**Observations, not failures:** stderr was 0 bytes in every run, so the log grep is trivially clean. Nine `-32700 Parse error` replies came from empty lines the operator sent. One `list_vault` pasted into the still-running first process answered from its frozen snapshot, as designed.

**Scope:** one run by one operator. It satisfies the “operator smoke test on op CLI 2.39.0 / macOS” half of the narrowed claim (Entry156). It does not qualify natural expiry, Linux or denial-format classification, and changes nothing about the abandoned calibration/continuity residuals. The checklist had been dry-run offline against the local-file backend before the user ran it; that dry run is not provider evidence.
