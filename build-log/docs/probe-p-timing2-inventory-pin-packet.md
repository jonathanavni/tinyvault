# Timing-2 inventory-pin packet (v4 — paper round 2 absorbed; returned to the user before dispatch) — pin the timing-2 report's exact test-title inventory in the execution gate

**Status:** **v4, 2026-09-10 — paper round 2 (Sol + blind Opus, on the committed v3 at `2b38dec`) absorbed: one convergent
P1 (the self-test construction), three verified P2s, eight P3s, one fourth residual named; every finding owner-verified and
dispositioned in §12; no mechanism changed. Nothing here is authorized for implementation.** **User decisions (2026-09-10): D-1 = Option A** — pin `scripts/test-execution.mjs` and
...
- **Claim, stated narrowly, with its four residuals.** (1) A registration that *runs* in the timing-2 partition and
  ...
  the lifecycle `it.each` statement (`testbed/probe/timingSourceContracts.ts:81-105` for the H titles;
  `timingSourceLifecyclePins()` at `:3` for the `it.each`); the other eight lifecycle registrations have no body pin on
  either side. No `CLAIM_LINKS` selector names the timing file, so `claim-execution` does not
  ...
  (4) **Hook bodies:** code in `beforeAll`/`afterEach`/`afterAll` of the timing-2 file produces no `assertionResults`
  row, so it never changes the multiset and is outside R1 by construction; the only control is the source-side root-hook
  pin (`timingSourceLifecyclePins().hook`/`rootHooks`), the layer C2 was narrowed for. The source scan and this rule are
  complementary; neither is universal registration isolation.
...
1. **The inventory.** `.vitest/timing-2.json` from the 2026-09-10 02:31 gate lists one file
   ...
   round-1 channels byte-compared this list to the report: exact ordered match. All 25 registrations (sixteen `it` plus eight `it` and one
   `it.each`) are unconditional string literals under two top-level suites (`host.timing.browser.test.ts:127-745`; no
   `skipIf`/`runIf`, no computed titles, no `retry`/`repeats` in `vitest.config.ts`), producing 26 report rows with no
   run-to-run variance. **Transcription source (both `.vitest/` and `artifacts/` are
   ...
   (SHA-256 `027d8453998d0e68bc72a140d22256bea3d9b3ef68408c102429d73117aec636`) **and, because a Codex worktree would
   not carry the untracked `artifacts/` tree, to the out-of-checkout location
   `/Users/jonathanavni/Documents/Coding/tinyvault-evidence/m7-packets-20260910/timing-2-green-20260910-0241.json`
   (same SHA-256)**; the dispatch names both absolute paths; the implementer transcribes the literal **from that report** (a `python3 -c` over `assertionResults[].fullName`),
...
2. **What the gate checks today.** `proveExecution` (`scripts/test-execution.mjs:54-65`) → `inspectReport` (`:38-52`)
   ...
   `docker-assertions` (`:47-48`, six names each exactly once `passed`, docker mode only — a **required-identity** rule,
   not a closed set: extra docker titles are accepted; R1 would be the gate's **first closed-set inventory** rule),
   `skip-identity` (one exact title) and `claim-execution` (`:148-157`, every `CLAIM_LINKS` runtime
...
3. **Consequence for the new rule's substance.** Because `assertion-status` + `skip-identity` already force every
   ...
   non-`passed` status, and a skipped row reds as `skip-identity` first (§4, M-T5 is the precedence witness). **The
   `mode === 'test'` guard is a second unkillable conjunct:** `partitions()` returns `[[DOCKER_TEST]]`/`[[EVAL_TEST]]` in
   the other modes (`:17-22`), so the `expected` keying already excludes them and no mutant can tell whether the guard
   is present; it is kept for readability. Stated plainly so no reader takes either conjunct as tested.
4. **The pinned root.** `testbed/rootOfTrust.test.ts:11-24` pins `package.json#scripts`, `Makefile`, the three Vitest
   ...
   `REQUIRED_DOCKER_ASSERTIONS` therefore lives in an unpinned module today (`test-execution.mjs:84`). Nuance (corrected in
   round 2): `TIMING_TESTS` and `REPORTS` are pinned **transitively** through `EXPECTED_TEST_COMMANDS` (`:18-20`) and
   `DOCKER_REPORT` through `:25`, which `check-test-entry.mjs:25-27` compares to the hash-pinned `package.json#scripts`;
   `DOCKER_TEST`, `EVAL_TEST`, `EVAL_REPORT` (the eval commands hard-code `.vitest/eval.json`) and the three start-file
   names are **not** (`DOCKER_TEST`'s value is separately cross-checked by the hash-pinned `vitest.config.ts` exclusion
   and by `report-files` in docker mode). The unpinned surface that matters is `test-execution.mjs`, which
...
7. **Rule precedence when the source scan is red.** The pinned-source checks run in **both** partitions — inside the
   timing file's first test (`host.timing.browser.test.ts:130-134`) and in the main partition
   (`testbed/probe/timing2Sidecar.test.ts:161-166`); if they red, `report-success` (`:42`) fires at bundle 0 (main) in
   the `bundles.reduce` at `:64`, before the timing-2 bundle is inspected, and again at bundle 2. The execution
...
- **A (recommended; both channels concur).** Literal in `test-execution.mjs` beside the docker literal; **widen the
  ...
  Decisions Log entry of 2026-09-05 (`PLAN.md:437`, which also carries the stale "both Vitest configs") enumerate the
  pinned set and gain "plus `scripts/test-contract.mjs` and `scripts/test-execution.mjs`" (comments: Astra; PLAN: owner,
  with "both" → "three" while touched). Recorded limits: A pins two *named* files, not "every
...
- **B.** Literal in `check-test-execution.mjs` (already pinned), passed to `checkExecution(root, mode, titles)`. No
  root-set change; but `filesystemCases` and `evaluationCases` (`test-execution.selftest.mjs:46,52,121,124,126`) call
  `checkExecution` directly while the CLI self-test spawns the real CLI (`gate-cli.selftest.mjs:116`), so a titles
  parameter would default to undefined and the self-test would exercise a rule the CLI does not (or none) — the proof
  weakens. Not taken (D-1).
...
- **Fixture.** `executionFixture` (`selftest:16-20`) keeps its groups; `report()` (`:7-15`) emits, for the
  ...
  one `test <file>` row.
- **Self-test construction (round 2, convergent P1).** `EXECUTION_MUTANT_CODES` is *derived* one code per
  `EXECUTION_MUTANTS` tuple and compared to `EXECUTION_RULES` by sorted-array `deepEqual` (`selftest:37,63`) — exactly one
  mutant entry per rule code — and `evaluationCases` re-applies `EXECUTION_MUTANTS` entries by code to the eval fixture
  (`:106-110`). So: **exactly one** new `EXECUTION_MUTANTS` entry, M-T1 (`['timing-2-inventory', (d) => { d.bundles[2]
  .report.testResults[0].assertionResults[3].fullName += ' hidden'; }]`), which supplies the code to the derived list
  (no hand-append); every other case lives in a new `timingInventoryCases()` called from `executionSelftest()` beside
  `evaluationCases()`/`claimLinkageCases()` (`:71-72`), each on a fresh `executionFixture()` with
  `assert.throws(() => proveExecution(d), { message: <code> })` and, for the green controls, a bare `proveExecution(d)`.
  Acceptance: `EXECUTION_MUTANT_CODES.length === EXECUTION_RULES.length === 22`.
- **Cases in `timingInventoryCases()`** (each named, each verified in memory by both round-2 channels against the real
  code with R1 inserted at the §2 placement): **M-T2** append a 27th passing row, counters coherent →
  `timing-2-inventory` (proves `report-counters` cannot mask it); **M-T3** remove one row, counters coherent →
  `timing-2-inventory`; **M-T4** duplicate one row, counters coherent → `timing-2-inventory` (the non-redundant multiset
  witness; a `Set` implementation survives it); **M-T6** the 26 titles under a different suite prefix →
  `timing-2-inventory`; **M-T9** append a 27th passing row and leave the counters **stale** → `timing-2-inventory` (the
  placement witness: with R1 after `report-counters` the same fixture reds `report-counters` instead). **M-T5**
  (precedence witness): one timing-2 row `skipped`, counters coherent → `skip-identity`. **M-T7** (ordering witness):
  **swap** `bundles[1]` and `bundles[2]` so `actualFiles` stays unique → `report-files` (`:45`), which precedes R1; a
  plain copy reds `partition-disjoint` (`:62-63`) before `inspectReport` runs — **M-T10** asserts exactly that
  (`partition-disjoint`) so a later refactor of `actualFiles` cannot silently move the boundary. **M-T8** (keying
  control, green): a coherent **27-row timing-1 bundle** (`TIMING_TESTS[0]`, 27 distinct titles) passes — R1 is keyed
  on the timing-2 partition; the untouched fixture (one timing-1 row) already witnesses this at `selftest:67`.
- **Mode isolation.** `modeFixture('docker')` and `modeFixture('eval')` stay green unchanged (`evaluationCases`, `:96`),
  and the eval-mode mutant loop is unchanged (M-T1's code is not in its filter list, `:107-108`). Docker/eval inertness
  is structural through the `expected` keying (§1.3) and is not claimed as a tested case.
...
- Under A, `testbed/rootOfTrust.test.ts:39`'s test title ("…three configurations and every check script") gains "and
  the two pinned identity modules", and its header comment `:1-2` names them.
...
- **V4** `npx tsc --noEmit`: PASS. **V4b** `node -e` over the selftest module: `EXECUTION_MUTANT_CODES.length ===
  EXECUTION_RULES.length === 22` and `'timing-2-inventory'` appears exactly once.
...
- **V6 (owner, real tree, after V1–V4 on the candidate):** (a) `make test` on the candidate: green, with the timing-2
  ...
  row with counters adjusted, (iv) duplicate one row with counters adjusted (the multiset property on real
  `fullName`s); after each, `node scripts/check-test-execution.mjs` must print
...
subagent edits; leave the work uncommitted. **STOP and report** if: the green report is not present at either path named
in the dispatch (§1.1; the out-of-checkout copy exists for a worktree) or its SHA-256 differs from §1.1; the report's 26 titles differ from §1.1 (drift); the rule cannot be
...
rule code observed | restored green`) covering M-T1–M-T10 and the 22 === 22 count; V1–V5 verbatim with the sandbox caveats; "Deviations From
...
**Round 2 (on v3 at `2b38dec`; Sol NEEDS-ATTENTION 1 P1 / 2 P3; blind Opus NEEDS-ATTENTION 1 P1 / 3 P2 / 6 P3; owner,
2026-09-10).** **Convergent P1:** §4 routed eight new cases through the existing `EXECUTION_MUTANTS` loop, but the code list is
derived one-per-tuple and array-`deepEqual`ed to `EXECUTION_RULES` (`selftest:37,63`) — duplicates break it — and the eval
filter (`:106-110`) would have re-applied M-T7 to the eval fixture; M-T8 (a green case) had no home. Absorbed: one
`EXECUTION_MUTANTS` entry (M-T1), `timingInventoryCases()` for the rest, the 22 === 22 acceptance (§4, V4b). **Opus P2s,
verified:** `docker-assertions` is required-identity, not closed-set (extras accepted) — §1.2; the `mode === 'test'` guard is a
second unkillable conjunct — §1.3; the transitive-pinning fact was wrong for `DOCKER_TEST`/`EVAL_TEST`/`EVAL_REPORT` — §1.4.
**P3s:** "two residuals" → four (Sol; the fourth, hook bodies, from Opus); 25 registrations / 26 rows; "nine" → eight;
lifecycle-pin citation; §1.7 the source scan runs in both partitions and `report-success` fires at bundle 0 first (Opus said
only the main partition — partly right; the timing file's first test also runs it, `host.timing.browser.test.ts:130-134`);
`gate-cli.selftest.mjs:116` spawns the CLI; `PLAN.md:437` "both" → "three"; `rootOfTrust.test.ts:39` title (both channels).
**Test gaps absorbed:** V6(b)(iv) duplicate on the real report; M-T10 split-`testResults` precedence; the optional docker case
no longer described as a case. **Residual absorbed:** the transcription source also copied outside the checkout for a
worktree, both paths in the STOP. Both channels: every mutant verdict reproduced in memory; real report green under R1; no
reaching shape; digest rows four; dispatchable after the P1 fix. No round-3 defect class remains open in the owner's
judgement; a cap round is the user's call.
