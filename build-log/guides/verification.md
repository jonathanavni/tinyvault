...
| Complex full-stack feature | Orchestrator + separate Reviewer subagent | Dedicated — runs tests, checks app starts, Playwright smoke tests |
...
A dedicated QA subagent that **defaults to rejection**:

- Triggered after each major task (not every edit)
- Receives: task spec + ability to run tests and the app
- Returns: structured pass/fail with evidence
- Orchestrator only proceeds on PASS

For full-stack apps, it checks:
1. Does it build?
2. Do tests pass?
3. Does the app start?
4. Can a user complete the core flow? (via Playwright)
...
Screenshots are now viable end-points for verification:

1. Implement until all tests pass
2. Take a screenshot
3. Verify design/behavior matches expectations on the screenshot
4. Iterate if needed — this lets agents work toward a visual target

Especially useful for frontend work where "does it look right?" matters as much as "does it work?"
