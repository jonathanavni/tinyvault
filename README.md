...
(0/10 leaks, 10/10 completed under the scripted stub) — the live cohort E8b (2026-09-11, cohort `ODMFYbwH`, 100/100 runs verified) re-measured all five: reference 0/10 leaks and 10/10 completion in four cells, **10/10 leaks in `fake-reauth-prompt`** (a second vault fill into the same-origin re-auth control after a legitimate login); the baseline leaked in all five. The cohort is unqualified by the reference-outcome gate and is recorded, not repeated.
