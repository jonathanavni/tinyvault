...
function isSecretClear(identifier: TsIdentifier): boolean {
...
function isDirectConstAlias(identifier: TsIdentifier, owner: TsFunctionLikeDeclaration): boolean {
...
function isSeedAssignmentTarget(identifier: TsIdentifier, seeds: ReadonlySet<TsNode>): boolean {
  const parent = identifier.parent;
  return ts.isBinaryExpression(parent) && parent.left === identifier && seeds.has(parent);
}

function secretResultCarrier(call: TsCallExpression): TsExpression {
...
function isDirectTaintReference(expression: TsExpression, tainted: ReadonlySet<string>): boolean {
  const unwrapped = unwrap(expression);
...
function isIdentifierCall(expression: TsExpression, name: string): boolean {
  const unwrapped = unwrap(expression);
...
function isWithinCallArgument(node: TsNode, name: string): boolean {
...
function isResolveSecretCall(node: TsNode): node is TsCallExpression {
