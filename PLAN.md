...
**Breadcrumb (2026-09-04, later):** slice A merged (`2e7b300`). **Slice B commit 2 fix round 2 (the last) at `b8066d7`**; round-3 verdicts: Codex NO-SHIP (streamed bodies = declared chunked; closed-page setup window = declared; requestId reuse P2), QA NEEDS-ATTENTION (P1 self-closing popup keepalive POST: neither body nor marker), security NO-SHIP (P1 unload-time beacon during navigation: no request event at all — declared M5-C7). **Integrator confirmation pass** in worktree `../tinyvault-wt-m5-B2pass` (now `50cac45` on the branch; worktree removed): marked provisional headers by identity + closed-target rejection fallback → `not-attached`; page-session body-fetch failure → `target-detached` marker, never `captureFailed`; lab routes `/popup-blob-close`, `/keepalive-navigate`, `/unload-beacon` (pins the M5-C7 declaration); 880 + 10 + 2 green, eval 30/30. **Commit 3 (the two hostile fixtures) at `8ed2737`** on `codex/m5-hostile-fixtures` (make test 871 + 10 + 2, eval 30/30 0 leaks, hostile.browser 7/7 after three integrator fixes: meta-gate multiple, stub turn cap 16, submitSelector for the three-button decoy page); round-1 review in flight — Codex NEEDS-ATTENTION (X1 M5-C2 missing from SCHEMA → task G; X2 exact-endpoint case bypasses the production adjudicator; X3 per-cell `bodiesUnobserved` unasserted; X4 C4 fabricates the surfaced-technique list), QA SHIP + 2 P2 (branch predates slice A → merge gate on the merged tree; C4 list fabricated), security SHIP + 2 P2 (C4 list; `max-turns` stop never checked → truncated scripts score green); fix round 1 (16 items, Codex) **landed at `0046ad0`** (881 + 10 + 2 green, eval 30/30, hostile 8/8; Codex's register/BACKLOG paragraphs parked in `m5-sliceB3-fix1-docs.diff` for main); **round-2 review in flight** — Codex `--base 50cac45`, QA/security worktrees `../tinyvault-wt-m5-B3f1-{qa,sec}`; C-B2f2 register section on main (`a98301f`). Merge note: the branch base (`0d82787`) predates slice A and the `scanTruncated` amendment; `git merge-tree` shows one conflict (`testbed/checkers/offline.ts`); `scanTruncated` per-cell assertions and the SCHEMA/README/spec text (drafts in `m5-schema-draft-decoders.md`, `m5-taskG-backlog-and-claims.md`) are applied at merge.
