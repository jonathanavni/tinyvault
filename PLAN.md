...
**Breadcrumb (2026-09-03, late):** **SLICE A MERGED to `main` (`2e7b300`)** — three review rounds + the integrator confirmation pass (register C-A1–C-A3); SCHEMA decoder inventory rewritten; spec sentence amended; BACKLOG residuals. Gate: 865 + 3 + 10 green, eval 10/10, `scanTruncated=0`. **Slice B commit 2 fix round 1** (uncommitted in `../tinyvault-wt-m5-fixtures`): integrator run green (839 + 10, eval 10/11 coverage), coverage repeats 9/10 → one test accepted only the `target-detached` marker string, fixed to the marker family; confirmation reruns ×5 + suite running (`sliceB2-fix1-run3.log`). Next: commit B2 fix → its round-2 three-channel review (packets `m5-sliceB2-fix1-review-packets.md`, base `034baff`) → then B commit 3 (`m5-sliceB-commit3-packet.md`, base = B2 fix tip) → merge B (rebase onto main) → task G docs for slice B (`m5-schema-draft-decoders.md` slice-B block, README coverage paragraph, plan §8 M5 ✅/§10). Worktrees to prune afterwards: `tinyvault-wt-m5-{A,A2,A3}-{qa,sec}`, `tinyvault-wt-merge-a`, `tinyvault-wt-m5-decoders`.
