...
**Breadcrumb (2026-09-03, late):** **SLICE A MERGED to `main` (`2e7b300`)** — three review rounds + the integrator confirmation pass (register C-A1–C-A3); SCHEMA decoder inventory rewritten; spec sentence amended; BACKLOG residuals. **Slice B commit 2 fix round 1 committed at `f745938`** (`codex/m5-hostile-fixtures`; integrator 839 + 10 green, eval 10/10 with coverage 10/11, coverage file 5/5 after the marker-family test fix) — **round-2 review: Codex NO-SHIP (a marker must never certify worker capture; correlation by URL+method; page-close), QA NEEDS-ATTENTION (converges; spurious markers for bodyless POST/DELETE; observations unbound), security pending** — the LAST commit-2 fix packet is drafted (`m5-sliceB2-fix2-packet.md`: delayed-fetch mechanism producers must yield the body, race producers reported; markers with `initiator: 'harness-marker'` and exact reasons; identity-keyed correlation reconciled after settle; no marker without a body) (Claude QA + security in `../tinyvault-wt-m5-B2f1-{qa,sec}`, Codex adversarial-review on `034baff..f745938`; buffer `m5-sliceB2f1-register-buffer.md` with the integrator series and the accepted schema departure). Next: absorb → (≤ 1 more fix round for commit 2) → B commit 3 (`m5-sliceB-commit3-packet.md`, base = the B2 tip) → its review → merge B onto main (rebase; SCHEMA/plan homes for `producerObservations`) → task G docs for slice B (`m5-schema-draft-decoders.md` slice-B block, README coverage paragraph, plan §8 M5 ✅ / §10). Remaining worktree to prune at the end: `tinyvault-wt-m5-fixtures` and the B2f1 review pair.
