...
**Breadcrumb (2026-09-03):** **slice A** round 2 (`8acff1d`) NO-SHIP on all three channels (count-cap invariant beaten twice; 8 KiB decode cap regression; raw-DEFLATE memory DoM) — register C-A2; **fix round 2 = the LAST round dispatched** (`task-mtkt29e8-u1wdbx` in `../tinyvault-wt-m5-decoders`, packet `m5-sliceA-fix2-packet.md`, P1 criteria stated). Contract amendment `scanTruncated` applied on `main` (suite running; commits on green). **Slice B commit 2** (`034baff`): Codex NO-SHIP (gate derives from in-memory objects; page-close test relaxed; nested wrapper replies can hang) + security NO-SHIP (worker bodies lost to Playwright's own resume — must become counted markers; detach-during-attach invalidates runs) — fix packet `m5-sliceB2-fix1-packet.md` drafted, waiting on the QA channel; register buffer `m5-sliceB2-register-buffer.md`. Next: B2 QA → C-B2 → dispatch B2 fix 1; A fix-2 lands → integrator run → round-3 review (last) → merge A (rebase onto the amended main) → then B commit 3.
