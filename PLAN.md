...
**Breadcrumb (2026-09-02):** paper ladder complete — r1 NO-SHIP (5 P1/3 P2), r2 NEEDS-ATTENTION (5 P1/1 P2), **r3 LOCKED at the cap** (`docs/m5-slice-spec.md`; register C-1/C-2). Next: `apply-m5-amendments.py` (scratchpad) + the mechanical code that keeps `main` green (empty `captureCoverage`, `bodiesUnobserved`, benign `stubScript`) → `make test` → amendment commit → worktrees → dispatch slice A (`codex/m5-leakscan-decoders`) ∥ slice B commit 1 (`codex/m5-hostile-fixtures`); packets in the scratchpad (`m5-sliceA-packet.md`, `m5-sliceB-commit{1,2,3}-packet.md`).
