...
- **Never name a zsh loop variable `path` (or `cdpath`, `fpath`, `manpath`) — they are tied to `PATH` etc.** `for path in …` in the E8b watcher clobbered `PATH`, the next `python3` call failed, and a `grep`-based liveness test then read "command not found" as "group gone" → the monitor exited 0 with the eval still running. Caught only because the local failure test asserted the group was killed. Pin `PATH` at the top of any monitor script and keep the liveness test free of external tools. (2026-09-11)
- **A background child started inside `$( … )` holds the substitution open until it exits.** `G=$(fake_group)` where the function does `perl … zsh -c 'sleep 300' &` blocked ~300 s, and the "test" then passed trivially against a dead group. Redirect the child's stdio (`>/dev/null 2>&1 &`) inside the substitution, and assert the target is alive right before the inducing action. (2026-09-11)
- **On macOS a dead or zombie-only process group prints nothing in `ps -o stat= -g <pgid>` and `kill -0 -- -<pgid>` fails with "no such process"** — so an unreaped zombie does *not* keep the group "alive" for `kill -0` on the *group* (it does for `kill -0 <pid>`). The only ambiguous state is "ps printed nothing but `kill -0 -- -pgid` succeeds" — treat it as an inspection error, not as dead. Probed 2026-09-11. (2026-09-11)
- **Backgrounded Bash tool commands can have shell variables mangled by the harness wrapper** (a `S=/path; … $S/file` chain ran with `S` empty and wrote to `/`). Use literal absolute paths in background commands, or a script file. Same family as the 2026-09-09 heredoc `${…}` re-quoting. (2026-09-11)
- **BSD `cp -R "$dir/"` copies the directory's *contents*, not the directory.** `e8b-preserve.sh` iterates `"$CLONE"/artifacts/eval/*/` (trailing slash) and `cp -R "$c" "$EVID/artifacts-eval/"` put the E8c cohort's files flat into `artifacts-eval/` with no `<cohort>/` subdirectory; the preserved-copy check then found no `cohort.json`. Detect with `ls artifacts-eval/` (should list only cohort ids); fix by moving the contents into `artifacts-eval/<cohort>/` and `diff -r` against the clone before the manifest. The script's bytes were left unchanged (pre-registered); strip the trailing slash or use `cp -R "${c%/}"` if it is ever revised. (2026-09-12)
