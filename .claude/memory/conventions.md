...
## Implementer STOPs on fixtures (from M6.1, 2026-09-09)

- **A STOP on an out-of-ownership test fixture is dispositioned by a scoped ownership extension, never by weakening the rule.** The AM12 cap-retention witness built synthetic baseline rows without the loop bootstrap a real run always carries; R1 correctly rejected them. Four inserted lines made the fixture production-shaped, cap and assertions preserved. A fixture that contradicts a production invariant is the fixture's defect. Record the extension in the packet's ownership list and in the implementer's report as history, not as a deviation.
- **Live-cohort monitors live gitignored beside the evidence, never in the repo, and are pre-registered by path.** The E8b watcher/launcher/go-wrapper/test suite under `artifacts/review-evidence/tinyvault-m7-e8b-20260910/` are the reusable set: the eval is started in its own process group via `perl -e 'setpgrp(0,0); exec @ARGV'`, the watcher kills that group on every loss-of-monitoring path, the launcher enforces a heartbeat deadline, and the ledger is reconciled against the harness's own `execution.usage` after the run (they matched to the token on 2026-09-11). A future cohort copies them into its own evidence directory and reruns the 34-assertion suite first. (2026-09-11)

## Paper-ladder conventions confirmed on the runtime-fill-control packet (2026-09-11)

- **The security third channel for a paper artifact is a fresh-context Opus review in attacker framing** (the prompt names the two adversary seats — prompt-injected model with the seven tools, hostile page on the authorized origin — and demands concrete ordered tool/page sequences with the deciding `file:line`). `/security-review` is diff-aware and has no diff to read before implementation. Run it as the third blind channel every round.
- **State the cap round's P1 criteria verbatim in the prompt and ask each channel for a cap verdict by criterion letter.** When channels disagree (2 PASS / 1 NEEDS-ATTENTION here), the owner adjudicates each P1 against the criteria, absorbs the in-criteria ones as the confirmation pass, and records the dissenting verdict unchanged in the register.
- **Decisions the owner can close from the code or from a locked doc's own amendment rule stay owner decisions (D-RC), overturnable and id-stable;** only boundary sentences, spend, locked-doc amendments and product semantics go to the user (O-RC). Two channels flagged the over-escalation; the demotion kept the ids so the register stays readable.
- **Every fact that counts something (tests to renew, runs that took a path) states what the count can and cannot show.** Rev 1's "0/50" and rev 2's "two extra units" each cost a round.

## Implementation-ladder conventions confirmed on the runtime fill-control slice (2026-09-11)

- **Three fix rounds, and the last round's P1 criteria in the dispatch packet and in every review packet.** Round 1 was a gate red (integrator hygiene, no dispatch); rounds 2 and 3 were Astra `task --fresh --write` on the same worktree, single writer. After round 3, every further static evasion of the pin was recorded under R19 and none opened a round; the final reviews were asked to judge against the stated criteria and all three PASSed.
- **Reviews and the mutant table run on the candidate SHA in parallel (read-only, isolated worktrees); the four gates run afterwards, alone, on the exact final SHA.** Earlier greens certify their own SHA only; every red is preserved with its log and JSON.
...
- **Repo layout since launch (2026-09-19)** — the root and `docs/` hold what a reader needs; the build record lives in `build-log/` with its old relative layout (`build-log/PLAN.md`, `build-log/docs/…`). New plans, packets, handoffs and registers go in `build-log/docs/`; new reader-facing docs and cited evidence go in `docs/` and get a line in `docs/README.md`. Paths written inside older records are relative to `build-log/`.
- **Public-repo voice** — the README is first person, plain words, no em dashes, no "X, not Y" flourishes, internal shorthand explained or left out; modelled on the owner's KuchiClaw README and blog. Every factual sentence in it was checked against code or a recorded result; keep it that way when editing.
