...
2. **Check working state** — `git status` + `git worktree list` to surface uncommitted work, active branches, and stale worktrees, plus any running background jobs. Apply [the shared ownership check](../../docs/handoff-pattern.md#one-writer-and-handover); do not take over an active or paused session implicitly. Explicit delegated packets retain worker scope.
...
- **Focus stamp (triggered, not a step):** once the session's direction is agreed and ownership is clear, record a one-line `` `YYYY-MM-DD-<thread>` — focus: <agreed direction> `` marker atop `PLAN.md` Current State, with `owner: claude`, `state: active`, and the checkout paths from the shared protocol, so an interrupted or compacted session leaves a breadcrumb. `/wrapup` fills in the outcome. Delegated workers return a report instead of stamping project state.
