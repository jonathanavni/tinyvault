...
- **Codex model = `gpt-6-astra`** (GPT-6 Astra) for every mode — reviews, implementation slices, and rescue. It is the built-in default of `adversarial-review` / `review`, and is passed explicitly as `--model gpt-6-astra` in `task` mode. **Requires codex-cli ≥ 0.153.3**, and an upgrade does not take effect until the shared runtime broker is restarted — see `.claude/memory/gotchas.md`. Prefer the native modes; `task --model` is the escape hatch when a mode's default model is unavailable.
