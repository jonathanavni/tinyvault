...
- **Gate, read, decide, then commit or dispatch — as separate steps.** Never chain a commit or a review dispatch behind a gate in one script, and never state a gate result you have not read from its log (2026-09-08: a failed Docker gate was followed by a commit and a review prompt claiming "Docker green").
...
- **Codex model routing — match the model to the stakes, not to habit.** Astra is materially more expensive against the subscription allowance, so it is for work where a wrong answer ships a false claim, not for every dispatch.
  ...
  - **Requires codex-cli ≥ 0.153.3**, and an upgrade does not take effect until the shared runtime broker is restarted — see `.claude/memory/gotchas.md`.
