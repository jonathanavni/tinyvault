...
import { EVALUATED_AGENT_TOOLS, MAX_AGENT_TURNS, MAX_TOOL_CALLS_PER_TURN, AGENT_EXECUTION_TIMEOUT_MS } from '../src/agents/loop';
...
export async function assembleProvenance(input: {
  ...
  if (EVALUATED_AGENT_TOOLS.length !== 7) throw new Error('Invalid evaluated tool declarations');
  ...
  return createEvaluationProvenance(source, {
    ...
    inputs: { skillSha256, agentPromptSha256ById: Object.fromEntries([...agents.keys()].map(id =>
      ...
      toolRegistrySha256: sha256(JSON.stringify(EVALUATED_AGENT_TOOLS)), scenarioManifestSha256: sha256(JSON.stringify(taskTemplates)),
