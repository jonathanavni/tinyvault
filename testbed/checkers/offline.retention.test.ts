...
it('AM12 retains all 60 verified cap-sized real-profile snapshots before recomputation', async () => {
  const root = await fs.realpath(await fs.mkdtemp(join(tmpdir(), 'tinyvault-am12-retention-')));
  ...
  const childDirectory = await fs.mkdtemp(resolve('.vitest/am12-child-'));
  ...
  await fs.rm(childDirectory, { recursive: true, force: true });
