...
// Deliberately independent of the protected supervisor matcher. metaGate.ts plants its own
// vectors (including independentBase32) and mutation-tests every enabled transform here.
function firstMatchingCheckerTransform(
  ...
  for (const name of SECRET_TRANSFORM_NAMES) {
    ...
    if (value !== undefined && matchesCheckerTransform(bytes, canary, name, value)) return name;
  }
  return null;
}

function matchesCheckerTransform(
  bytes: string,
  canary: string,
