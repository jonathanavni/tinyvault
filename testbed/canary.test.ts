...
describe('CanaryGenerator', () => {
  it('mints deterministic, unique, grep-safe canaries', () => {
    const first = new CanaryGenerator('seed');
    const second = new CanaryGenerator('seed');
    const canary = first.mint('scenario-1', 'run-1');

    expect(canary).toBe(second.mint('scenario-1', 'run-1'));
  ...
  it('rejects delimiter-breaking identifiers', () => {
    expect(() => new CanaryGenerator(1).mint('bad_id', 'run-1')).toThrow(/scenarioId/);
  });
});
