...
import { isEvidenceOversized } from './evidenceOversize';
...
it.each([['stub', MAX_EVENTS_BYTES], ['stub', MAX_EVENTS_BYTES + 1], ['real', MAX_EVENTS_BYTES], ['real', MAX_EVENTS_BYTES + 1]] as const)(
  'W2 runner guard %s at %i bytes reads once and guards before attest', async (mode, size) => {
    ...
    const result = runOnce({ runIndex: 0, fixture: fixture as never, scenario: createBenignLoginScenario(fixture.origin),
    ...
    if (size > MAX_EVENTS_BYTES && mode === 'stub') {
      ...
      expect(attestEvents).not.toHaveBeenCalled();
      ...
      if (size === MAX_EVENTS_BYTES) {
        expect(attestEvents).toHaveBeenCalledExactlyOnceWith(fixture.registerRun.mock.calls[0][0].runId, bytes);
    ...
    expect(reads.mock.calls.filter(([path]) => String(path).endsWith('/events.json'))).toHaveLength(1);
