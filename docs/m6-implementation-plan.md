...
**M6 CLOSED 2026-09-09 (owner note):** S4, S5 and S6 were accepted after the status paragraph below was written; it is preserved as the S3-era record. Current state: §3 (AM11–AM13, F1 rows), the M6 register, and [`project-assessment-2026-09-09.md`](project-assessment-2026-09-09.md).
...
D-CANCEL remains OPEN before S4 dispatch (§8) *[resolved 2026-09-07, `2bcfbbd`]*. S2 implementation was separately authorized in the fresh
...
bounds or previously accepted S1/M5 limits. D-CANCEL remains OPEN *[resolved 2026-09-07, `2bcfbbd`; S4 accepted]*.
...
S4 is implemented and accepted at the round-3 cap (2026-09-07). S5 is implemented and accepted at the round-3 cap with the integrator confirmation pass (2026-09-08; register entry "S5 implementation — accepted", with declared residuals). S6 (E9/E10) is accepted (2026-09-09): three literal clean-clone gates on `3072e0b`, pilot `cY3Deep4` READY under the AM13 fail-closed rule, N10 sequence `E9-A3-N10` QUALIFIED (baseline `z22Kn2eT`, comparison `y9WmFqoL`), E8 met, E9 met, E10 recorded — register `m6-review-findings.md` and the [milestone-close assessment](project-assessment-2026-09-09.md).
...
| M6-AM12 | Raise the raw signed-events cap 131072 → 1048576 and the outer bridge frame ceiling 262144 → 2097152; give the five bridge scalars that inherit the frame ceiling explicit bounds (receipt/attest strings stay 262144; key/hello public keys and capture chunks type-sized); amend AM11 item 3 (known serial witnesses become fit/attested, the 1,409,051-byte maximum-output trace stays the rejection witness) and the §4.3 gate values; no gate added or loosened; companion S5-return `evidence-oversized` diagnostic packet lands first; full E9 rerun from a clean clone with attempt 1 preserved and disclosed. Full text, evidence and residuals: `docs/m6-am12-events-cap-amendment.md`. | User APPROVED 2026-09-08 ("I agree with your recommendations") after the three-round paper cap (Sol R1, Opus 5 R2, Sol R3) — 1 MiB over 512 KiB; the disclosed frame-ceiling side effect accepted; companion packet sequenced first. ADOPTED as policy; implementation is a separate full-ladder Astra slice (constants, tests, P-v2 claim-row packet, plan §4.3 gate text). Implemented and ACCEPTED at the round-3 cap, merged `623a8b7` (2026-09-08; register entry "M6-AM12 implementation slice"). |
| M6-AM13 | Separate N=1 pilot readiness from N10 outcome qualification: only N=10 qualifies on the real command path (`pilot-not-qualification`); pilot readiness is a fail-closed owner record on exact qualification reasons; one defined pilot-progression exception (one or more verified baseline noncompletions with their leaks observed, everything else READY) permits exactly one pre-declared N10 sequence; the baseline recovery instruction gains a `/success`-path completion check (disclosed protocol change; attempts 1–2 non-comparable). Supersedes the §4.3 "every real pilot must reach its expected end" sentence and the AM11 "No adjustment after a pilot/cohort failure" sentence to exactly the bracketed extent. Full text: `docs/m6-am13-pilot-qualification-amendment.md`. | User ADOPTED 2026-09-09 with corrections A (fail-closed readiness) and B (full ladder). Implemented and accepted at review round 2, merged `fe8e9e1` (2026-09-09); pilot 3 `cY3Deep4` READY and N10 sequence `E9-A3-N10` qualified on `3072e0b` (see `m6-review-findings.md`). |
| M6.1 | Receiptless-row canary authentication (M6C-CODEX-P1-01): every `naive-baseline` row's manifest canary must equal the `password` in its single fixture-signed loop bootstrap event (selected by the four stable fields, either loop initiator), checked in `recomputeRun` before the receipt and before any canary search; per-run `signature-mismatch`; composed tamper witnesses for both real profiles; `P-receipt-binding` row and SCHEMA s115/s116 amended. Packet `m6-1-canary-authentication-packet.md` v4 ADOPTED at the three-round Sol paper cap; Astra-implemented on `codex/m6-1-canary-auth`; three-channel post-implementation review; owner `make test` and `make test-docker` on the candidate. Register entry "M6.1 receiptless-row canary authentication". |
...
resampled or counted as completed. [M6-AM13 exception, user-approved 2026-09-09: the sole exception is one or more intact, verified BASELINE runs that did not complete (`taskCompleted: false`, no authorized canary) with the required non-benign leak observed, while every reference cell and every remaining baseline cell is READY; it permits exactly one pre-declared N10 sequence, and only when the pilot's qualification reasons are exactly ['positive-control-missing'] with every missing-control cell attributable exclusively to those baseline runs; any other or unknown reason is NOT READY. Reference noncompletion, a missing baseline leak, any evidence failure or any oversize never advance.] Any proposed bounded-artifact protocol
...
2. Fix the finite witness schedules before implementation: benign/DOM-hidden response call counts
   ...
   provenance binding and disclosure with results. No adjustment after a pilot/cohort failure. [M6-AM13, user-approved 2026-09-09, is the sole one-time exception: after attempts 1–2 and before pilot 3, the baseline recovery instruction changes as disclosed in AM13 §4.3 and the real-path N=10 qualification gate of §4.1 is added. No prompt, configuration, fixture, scenario, cap or gate change is permitted after pilot 3 begins or during its N10 cohorts; a further change requires a new user-approved amendment and a new labelled attempt.]
3. Replace the ambiguous universal reading of the last paragraph's normal-success STOP with this exact
   ...
   remains unqualified/nonzero, and cannot be dropped, replaced, resampled or counted as completed.** [M6-AM13 exception, user-approved 2026-09-09: the sole exception is one or more intact, verified BASELINE runs that did not complete (`taskCompleted: false`, no authorized canary) with the required non-benign leak observed, while every reference cell and every remaining baseline cell is READY; it permits exactly one pre-declared N10 sequence, and only when the pilot's qualification reasons are exactly ['positive-control-missing'] with every missing-control cell attributable exclusively to those baseline runs; any other or unknown reason is NOT READY. Reference noncompletion, a missing baseline leak, any evidence failure or any oversize never advance.] The
...
| S5 Composed real-agent command path | `testbed/runner.ts`, `testbed/runnerExecution.ts`, `testbed/evalEntry.ts`, `testbed/evalEntry.test.ts`, `testbed/evalAgents.ts`, `testbed/scorecardAggregate.ts`, `testbed/runner.test.ts`, `testbed/runner.testkit.ts`, `testbed/runner.inProcess.test.ts`, `testbed/runner.browser.test.ts`, `testbed/runner.artifacts.test.ts`, `testbed/runner.wiring.test.ts`, `testbed/runner.eval.test.ts`, `testbed/runner.realAgent.test.ts` (new), `testbed/runner.realAgent.eval.ts` (new, non-test module reached from the single pinned eval test; owner decision D-S5-9, 2026-09-07 — a second `*.test.ts` eval file would trip the pinned eval partition and the single-skip rule), `testbed/cohort.ts`, `testbed/realAgentRun.ts`, `testbed/sourceInventory.ts` (new, each with a sibling `.test.ts`; D-S5-5), `package.json`, `Makefile`, `vitest.eval.config.ts`. Pre-authorized Astra-only extensions (D-S5-6/7/10): `src/supervisor/host.ts` (trusted `setupReasonFor`), `src/supervisor/evidenceLease.ts` (abort evidence snapshot + accessor), `src/agents/anthropicClient.ts` (exported resolved client config read by the class). Packet: `docs/m6-s5-handoff.md`. E7/E8, agent/cohort-scoped run identity, runId-bound start/recovery URL and recipe wiring, AM09/AM10 diagnostic retention and failed-cohort/per-cell qualification reporting. Re-prove E1–E4 at the actual command boundary. | S4; high risk gating, full ladder. Cap-round integrator fix returned UNCOMMITTED 2026-09-08: a verifying events attestation is minted only after successful finalization and is required for every numeric row; an empty attestation excludes the real run before any events read. Diagnostic usage aggregates saturate at Number.MAX_SAFE_INTEGER. H1 command-path failure/promotion witnesses and H2/H3 containment/guard tests accompany the prior shared acceptance/pairing/snapshot rules. Three review rounds consumed; owner make test and integrator confirmation (not a fourth review round), owner docs/claims integration and S6 acceptance remain due. |
