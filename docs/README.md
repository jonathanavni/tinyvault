...
- [`m9-onepassword-packet.md`](m9-onepassword-packet.md) — revision7 D1–D9/offline S1–S4 implemented and current offline gates pass (Entry65); M9 acceptance pending. Three implementation review rounds and all literal verdicts/limits, five historical paper rounds and Entry7 conditions retained. The separately approved M5 marker-wait repair is verified with full default gate and three scoped reviews passing (Entry70); historical failure causes remain unproven. Candidate `7c400ab` and literal clean-clone default gate complete (Entry73); V1/expiry, supported-OS qualification, integration and audit remain pending. [Per-user setup](onepassword-setup.md); decisions and exact evidence in the M9 register.
