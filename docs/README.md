...
- [`probe-p-timing2-policy.md`](probe-p-timing2-policy.md) — Probe P timing-2 gate policy: v2 decision note (gate unchanged until the user decides otherwise; observability first as C1 landed / C2 held; a pre-registered N = 20 `make test` characterization campaign with a run-count decision rule whose every outcome is a proposal to the user). **Awaiting user adoption**; two blind paper reviews dispositioned in the M7 register.
