...
- [`m8-mcp-adapter-packet.md`](m8-mcp-adapter-packet.md) — M8 MCP stdio adapter plan-mode packet, **rev 5.2 LOCKED by the user 2026-09-12** after the three-round paper ladder (revs 1–4) and the rev 4 → 5 contract correction (modern `_meta` keys reconciled, the authorized Claude Code client probe, O-1 resolved on measurement, §3.11 carrying the runtime fill control's §6.6 (0)–(6)/O-RC-3/O-RC-8/O-E8c-1 with the stated restart-authority limitation; two capped Sol delta rounds); **implemented under approved C1–C5; reviewed and gated at the round-3 cap; merged locally as `412b6687` after user approval; push blocked by a red merged-tree M6 finalization test** — see the register for the actual gate failure and proposed separate test repair; `m8-astra-implementation-handoff.md` remains the historical implementation handoff. No push, cohort or public flip.
...
- [`m8-astra-implementation-handoff.md`](m8-astra-implementation-handoff.md) — **implementation handoff (2026-09-12), execution in progress** in the Codex continuity session implementing M8 under the locked packet rev 5.2: base SHA, accepted decisions, completed reviews and caps, evidence paths, required gates, the final interoperability check with a throwaway vault (a verification of the stated limitation, not a security claim), pending work and authorization boundaries.
