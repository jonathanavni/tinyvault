...
- [`m8-mcp-adapter-packet.md`](m8-mcp-adapter-packet.md) — M8 MCP stdio adapter plan-mode packet, **rev 4 lock candidate (2026-09-10)** after the three-round paper ladder; user decisions on O-2…O-8 recorded in `PLAN.md`; O-1 pending the modern `_meta` reconciliation and the authorized client probe; **not locked, no implementation dispatched**.
...
- [`m7-e8c-live-cohort-proposal.md`](m7-e8c-live-cohort-proposal.md) — **PROPOSAL (2026-09-11), not a pre-registration, nothing authorized:** the post-merge live qualification cohort (candidate `main` `6812627`, unchanged qualification rules, ≈ $3.2 with the E8b-style $10 stop threshold as a proposal, the packet §8.3 transcript-derived fill-authorization checks); the next session writes `m7-e8c-live-cohort-preregistration.md` rev 1 from it and returns it for spend approval.
- [`m7-runtime-fill-control-packet.md`](m7-runtime-fill-control-packet.md) — **rev 3.1, IMPLEMENTED AND MERGED 2026-09-11** (`main` `6812627` = candidate `f1dc22c`; full ladder, three fix rounds, all gates green, three final channels PASS; live qualification pending; register "Runtime fill-control — implementation slice" and "MERGED"): runtime fill-control design packet (bounded fill authorization vs completion-triggered revocation), **rev 3 REVIEWED PROPOSAL (2026-09-11)** after the three-round paper ladder (Sol + blind Opus QA + blind Opus security per round; cap: 2 PASS, 1 NEEDS-ATTENTION absorbed); recommends design A; the handoff's failed-login-retry sentence is an **open user decision (O-RC-6)**; **not locked, no implementation dispatched, no spend**. Register rounds 1–3 in `m7-review-findings.md`.
