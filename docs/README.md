...
- [`m8-mcp-adapter-packet.md`](m8-mcp-adapter-packet.md) — M8 MCP stdio adapter plan-mode packet, **rev 4 lock candidate (2026-09-10), NOT LOCKED** after the three-round paper ladder; user decisions on O-2…O-8 recorded in `PLAN.md`; **rev 5 owed** — the runtime-authorization requirements of `m7-runtime-fill-control-packet.md` §6.6 (0)–(6) and O-RC-8 must be incorporated (handoff in `PLAN.md` Current State); O-1 pending the modern `_meta` reconciliation and the authorized client probe; **not locked, no implementation dispatched**.
- [`m8-review-findings.md`](m8-review-findings.md) — append-only M8 register: paper rounds 1–3 (Sol read-only + blind Opus each), every finding's verification and disposition, the gate mechanism probe, residuals.
