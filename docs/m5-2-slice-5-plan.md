...
| Fixture observes admin entry, requires finalized run, consumes once, caps events at 131072 bytes [values amended by M6-AM12, 2026-09-08] | `shared/loginFixture.ts`, `attestFixtureEvents` | Retain lifecycle; pass fixture identity from private state |
...
| Bridge validates canonical outer frames and scalar artifact strings up to 262144 bytes, including Unicode; inner artifact JSON/signature schema remains opaque [values amended by M6-AM12, 2026-09-08] | `docker/protocol.ts`, `handshake.ts:102-103`, `frames.ts` | Add inner artifact validation to direct signature producers/consumers |
...
The serialized artifact is at most 262144 UTF-8 bytes on both producer and consumer. This is a new [values amended by M6-AM12, 2026-09-08]
Slice5 implementation bound supporting D4's bounded artifact handling, not an inherited limit or new
trust mechanism. `handshake.ts:102-103` already enforces the SAME 262144 scalar byte bound on [values amended by M6-AM12, 2026-09-08]
