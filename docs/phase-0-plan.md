...
| M8 | MCP stdio adapter (mutex-guarded) | `adapters/mcp/server.ts` | MCP client lists tools; fill via MCP goes through the same gate + choke-point; no secret in any MCP result; **[pre-impl review #1] MCP results traverse the same capture/tripwire seam as direct calls** — proved by test, not assumed by construction | 🔴 **packet rev 5.2 locked 2026-09-12** (`docs/m8-mcp-adapter-packet.md`; implemented under C1–C5; candidate reviewed and gated at the round-3 cap; merged locally `412b6687`; push and closure blocked by red merged-tree M6 finalization test, `docs/m8-review-findings.md`) |
