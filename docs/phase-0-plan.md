...
3. **Out-of-band content tripwire (instrumentation ONLY — never a runtime gate, round-2 #2; plane-split by pre-impl review #1).** The earlier wording promised both "fails the eval/CI run" and "never alters process lifetime," which cannot both hold in one process — a test runner that fails *does* exit nonzero. That is an **architecture gap, not a fatal contradiction**: it resolves once **caller-visible** and **eval-visible** are separated.
   ...
   **The supervisor-facing seam is an EXACT API, not a label** (round-2 #1 — an unpinned "seam" can later be satisfied by an `onMatch(callback)` or a synchronous `scanAndReport()` reachable from the data plane, which re-opens the very callback oracle this layer bans). Pinned shape: **sealed batch in → verdict + fixed-shape diagnostics out.** It is **control-plane-only**: no callbacks, no mutation of caller state, no per-event data-plane hook, and it **refuses caller-originated or mixed-provenance bytes** at the type level rather than by convention. M2 must carry a test or type-check proving the seam cannot be used as a callback sink in the caller path.
...
- The content tripwire is **instrumentation, not a runtime gate**: it produces protected diagnostics and the eval/CI verdict from the control plane, and never changes caller-facing bytes, error paths, session state, mutex behavior, or control flow (§4 layer 3 plane split). On timing the honest claim is bounded, not absolute: it **does not produce a detectable timing difference under the locked repeated-trial probe and threshold** (round-2 #3 — "never changes timing" is unprovable and was withdrawn). Reflection is handled by provenance-masking (§4 layer 2), not by comparing DOM content to a secret.
