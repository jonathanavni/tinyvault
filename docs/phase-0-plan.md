...
| M2 | **Security primitives (unit-testable in isolation — round-2 #6)**: `Secret<T>` wrapper, bare-origin string validator, taint/lockdown registry, session mutex, exact result constructors, the content tripwire as pure instrumentation, **[B1 slice 1/3] `Secret<T>` no-plaintext-retention lifecycle** (explicit clear/consume; no plaintext or derived material survives use in primitive state) | `core/redaction.ts`, `core/originGuard.ts`, `core/lockdown.ts`, `core/sessionMutex.ts`, `core/results.ts` | pure-unit tests with NO browser: `Secret` masking, origin-string accept/reject, noninterference differential (`results.ts`), tripwire-fires-without-caller-visible-change, mutex mutual-exclusion, taint registry lock/unlock; **[B1 slice 1/3] post-use `Secret<T>` retains no plaintext** — after clear/consume, masking holds and no accessor, property enumeration, serialization, or error path yields the value (pure unit, no browser). **No test here asserts a real-fill/DOM property** — those move to M4; in particular B1’s "a second fill re-resolves" is **M4**, and the backend never-cache contract is **M3**. | 🔴 |
